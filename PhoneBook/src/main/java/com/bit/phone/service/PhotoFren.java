...
public class PhotoFren {
	public void fileload(HttpServletRequest request,Fren_inf mmbr) throws IllegalStateException, IOException {
		...
		if (!mmbr.getFilename().isEmpty()) {
			...
			System.out.println("-------------------------------------");
			System.out.println(mmbr.getFren_nm());
			System.out.println("-------------");
			System.out.println(mmbr.getFilename());
			String fileName = mmbr.getFren_nm() + "_" + mmbr.getFilename().getOriginalFilename();// id값을
																									// fileName으로
																									// 줄것임
			System.out.println(mmbr.getFilename().getOriginalFilename());
			mmbr.getFilename().transferTo(new File(dir, fileName));

			mmbr.setFren_photo(fileName);// 새롭게 만들어진 photoName을 setPhoto로
